...
public class Thread_move extends Thread{
	...
	public void run(){
		...
		double[] ms = {5.97E10,6.39E11,7.34E8,50,35,6,80,20,50,35}; //array di masse
		...
		while(!collision){
			...
			for(Planet pl:planets){
				//trovo la direzione della forza risultante che dipende dalla posizione del pianeta rispetto agli altri
				direction = pl.getTotalForces().getDirX() + pl.getTotalForces().getDirY();
				
				//calcolo le due componenti dell'accelerazione e velocità
			...
			//calcolo le nuove forze dopo lo spostamento e la risultante
			for(Planet pl:planets){
				force = new Force[planets.size()-1];
				k=0;
				for(Planet plo:planets){
