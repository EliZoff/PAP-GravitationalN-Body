...
public class Thread_move extends Thread {
	...
	public void setGraphicComponents(JFrame f, /* JPanel panel, */Box b) {
		this.f = f;
		this.box = b;
		// this.panel = panel;
	}

	public void setDrawer(My_panel p) {
		this.p = p;
	}
	...
	public void genPlanets(int nPl, double[] m, int[] r) {
		...
		newP.setSize((f.getWidth() - 150), (f.getHeight() - 39));
		this.setDrawer(newP);
		f.validate();
			...
			if (restart) {//USARE UN SEMAFORO
				...
				while (!collision) {
					...
					for (Planet pl : planets) {
						...
						direction = pl.getTotalForces().getDirX()
								+ pl.getTotalForces().getDirY();
					...
					for (Planet pl : planets) {
						...
						for (Planet plo : planets) {
							if (!pl.equals(plo)) {
								...
								if (!collision) {
									collision = checkCollisions(pl.getX(),
											plo.getX(), pl.getY(), plo.getY(),
											pl.getR(), plo.getR());
								}
								k++;
							}
						...
						totFval = Math.sqrt(Math.pow(fx, 2) + Math.pow(fy, 2));
						// e la imposto nell'oggetto
						pl.setTotalForce(new Force(totFval, fx, fy, dirFx,
								dirFy));
